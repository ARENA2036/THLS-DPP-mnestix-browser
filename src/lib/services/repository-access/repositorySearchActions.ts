...
import { AssetAdministrationShellRepositoryApi, SerializationApi, SubmodelRepositoryApi } from 'lib/api/basyx-v3/api';
...
export async function getThumbnailFromShell(
    ...
    const fileSearcher = AssetAdministrationShellRepositoryApi.create(baseRepositoryUrl, mnestixFetch(null));
...
export async function getAttachmentFromSubmodelElement(
    ...
    if (baseRepositoryUrl) {
        const fileSearcher = SubmodelRepositoryApi.create(baseRepositoryUrl, mnestixFetch(null));
...
export async function downloadAasFromRepo(
    ...
    const fileSearcher = SerializationApi.create(baseRepositoryUrl, mnestixFetch(null));

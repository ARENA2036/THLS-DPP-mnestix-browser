...
export class RepositorySearchService {
    ...
    static create(log?: typeof logger): RepositorySearchService {
        ...
        return new RepositorySearchService(
            prismaConnector,
            (baseUrl) => AssetAdministrationShellRepositoryApi.create(baseUrl, mnestixFetch(null)),
            (baseUrl) => SubmodelRepositoryApi.create(baseUrl, mnestixFetch(null)),
            searcherLogger,
        );
    }
    ...
    async getAasRepositories() {
        ...
        try {
            const connections = await this.prismaConnector.getConnectionDataByTypeAction({
                id: '0',
                typeName: 'AAS_REPOSITORY',
            });
            repositories = connections.map((c) => c.url);
        } catch (error) {
    ...
    async getSubmodelRepositories() {
        ...
        return connections.map((c) => c.url);
    }
    ...
    async getAasFromDefaultRepo(aasId: string): Promise<ApiResponseWrapper<AssetAdministrationShell>> {
        ...
        return wrapErrorCode(ApiResultStatus.NOT_FOUND, 'AAS not found', undefined, response.httpStatus);
    }
